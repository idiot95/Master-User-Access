...
export async function fetchManifest(moduleKey, baseUrl, { revalidate = 900 } = {}) {
  ...
  let res;
  try {
    res = await fetch(url, { redirect: "error", next: { revalidate, tags: [`manifest:${moduleKey}`] } });
  } catch (e) {
    return { ok: false, status: "Unreachable", error: String(e.message || e) };
  }
